...
class GlobalFilter(nn.Module):
    ...
    def forward(self, x, spatial_size=None):
        ...
        complex_weight = self.complex_weight if self.complex_weight is None or self.complex_weight.fast is None else self.complex_weight.fast
...
class BlockLayerScale(nn.Module):
    ...
    def forward(self, x):
        gamma = self.gamma if self.gamma is None or self.gamma.fast is None else self.gamma.fast

        x = x + self.drop_path(gamma * self.mlp(self.norm2(self.filter(self.norm1(x)))))
        return x
...
class GFNet(nn.Module):
    ...
    def forward_features(self, x):
        ...
        pos_embed = self.pos_embed if self.pos_embed is None or self.pos_embed.fast is None else self.pos_embed.fast
